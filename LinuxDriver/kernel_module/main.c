...
{
  printk(KERN_INFO LOG_PREFIX "write\n");
  return -EFAULT;
}
...
static int __init accDriverInit(void)
{
  major_num = register_chrdev(0, DEV_NAME, &fops);
  if (major_num < 0)
  {
    printk(KERN_INFO LOG_PREFIX "registering char dev failed! err: %d\n", major_num);
    return major_num;
  }
  printk(KERN_INFO LOG_PREFIX "Init with major number: %d\n", major_num);
  return 0;
}

static void __exit accDriverExit(void)
{
  unregister_chrdev(major_num, DEV_NAME);
  printk(KERN_INFO LOG_PREFIX "Exit\n");
}

module_init(accDriverInit);
module_exit(accDriverExit);
